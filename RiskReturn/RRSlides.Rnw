...
\item Keynes (1936)
\item Nassim Telab (2007): \emph{Black Swan}
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{block}{}
\begin{equation*}
variance = \frac{\sum_{t=1}^T (BACR_t - \mu_{BACR})^2}{T - 1}
\end{equation*}
\end{block}
\pause
\vspace{1cm}
Where $\mu_{BACR}$ is the mean of Bank of America returns.
\end{frame}
...
\begin{align*}
sd =& \sqrt(var)\\
sd =& \sqrt{\frac{\sum_{t=1}^T (BARC_t = mu_{BACR})^2}{T-1}}\\
\end{align*}
\end{block}
\end{frame}
...
legend("topleft", legend = c("property"), fill = colors[2])
@
\end{frame}
...
#lines(density(cash, adjust = 2), col = 'black')
legend("topleft", legend = c("property", "stocks"), fill = colors[c(2, 3)])
@
\end{frame}
...
legend("topleft", legend = c("property","stocks","bonds"), fill = colors[c(2, 3, 4)])
#lines(density(cash, adjust = 2), col = 'black')
@
\end{frame}
...
legend("topleft", legend = c("property",  "stocks", "bonds", "cash"), fill = colors[c(2, 3, 4, 5)])
@
\end{frame}
...
plot(Risk, Return, ylim = c(0, 0.15), pch = 3)
...
\begin{itemize}[<+-| alert@+>]
\pause
\item Have one portfolio of \textbf{high risk} and one portfolio of \textbf{low risk}.  Do asset returns match the theory? 
\item How do you determine risk? 
\item Here is one calculation
\item We will look at others in the coming weeks
\item Does not just have to be stocks
\begin{itemize}
\item Also other currencies (different rates)
\item Commodities
\item Property in different countries
\end{itemize}
\end{itemize}
\end{frame}
\end{document}